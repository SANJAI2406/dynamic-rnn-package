...
class RNNTab(ctk.CTkFrame):
    ...
    def _on_mode_change(self, choice):
        ...
        if choice == "Scalar":
            ...
            # 1. Restore buttons
            self.recompute_graphics_button.configure(state="normal" if self.trained_models else "disabled")
            ...
            # 4. Remove scalar-only tabs (WASTED SPACE FIX)
            try:
            ...
            # 5. Show info message (only if data isn't already loaded and dynamic)
    ...
    def _build_dynamic_models(self):
        ...
        try:
            ...
            self.build_model_button.configure(text="Build Models", state="normal")
            # Keep other buttons disabled as they don't apply to dynamic mode
            self.recompute_graphics_button.configure(state="disabled")
            self.run_shap_button.configure(state="disabled")
